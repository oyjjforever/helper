...
public class ListUtil {
     ...
     *            asc升序，desc降序
     */
    public static <E> void sort(List<E> list, final String orderMode, final String... fieldNames) {
            ...
            public int compare(E a, E b) {
                ...
                try {
                    for (int i = 0; i < fieldNames.length; i++) {
                        ret = ListUtil.compareObject(fieldNames[i], orderMode, a, b);
                        if (0 != ret) {
                            break;
                        }
                    }
     ...
     *
     * @param list
     * @param fieldNames  排序字段数组
     * @param orderMode      每个属性对应的升降序数组， asc升序，desc降序
     */

    public static <E> void sort(List<E> list, final List<String> fieldNames, final List<String> orderMode) {

        if (fieldNames.size() != orderMode.size()) {
            ...
            public int compare(E a, E b) {
                ...
                try {
                    for (int i = 0; i < fieldNames.size(); i++) {
                        ret = ListUtil.compareObject(fieldNames.get(i), orderMode.get(i), a, b);
                        if (0 != ret) {
                            break;
                        }
                    }
     ...
     * @throws Exception
     */
    private static <E> int compareObject(final String fieldName, final String orderMode, E a, E b) throws Exception {
        int ret;
        Object value1 = ListUtil.forceGetFieldValue(a, fieldName);
        Object value2 = ListUtil.forceGetFieldValue(b, fieldName);
